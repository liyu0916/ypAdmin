...
/**
 * Helper class that contains variuos counters that are local to the v-charts.
 */
 ...
 * given v-charts's animation option
	...
	win.HighchartsAdapter = win.HighchartsAdapter || ($ && {
		...
		init: function (pathAnim) {
			...
			$.fn.highcharts = function () {
				...
				// When called without parameters or with the return argument, get a predefined v-charts
				if (options === UNDEFINED) {
					ret = charts[attr(this[0], 'data-highcharts-v-charts')];
				}
...
var Color = function (input) {
	...
	function get(format) {
		...
		// it's NaN if gradient colors on a column v-charts
...
SVGElement.prototype = {
	...
	*/
	 ...
	 * pie slices regardless of positioning inside the v-charts. The format is
	 ...
	 * to x and y relative to the v-charts.
	 *
...
function StackItem(axis, options, isNegative, x, stackOption, stacking) {
	...
	// if the v-charts is inverted or not.
...
StackItem.prototype = {
	...
	setOffset: function (xOffset, xWidth) {
		var stackItem = this,
			...
			y = axis.translate(this.percent ? 100 : this.total, 0, 0, 0, 1), // stack value translated mapped to v-charts coordinates
...
Axis.prototype = {
	...
	/**
     * Remove the axis from the v-charts
     */
	...
	/**
	 * Translate from axis value to pixel position on the v-charts, or back
	 *
	 */
	translate: function (val, backwards, cvsCoord, old, handleLog, pointPlacement) {
		...
		if (backwards) { // reverse translation
			...
			val -= minPixelPadding;
			returnValue = val / localA + localMin; // from v-charts pixel to value
	 ...
	 * @param {Boolean} paneCoordinates Whether to return the pixel coordinate relative to the v-charts
	 ...
	 * @param {Boolean} paneCoordiantes Whether the input pixel is relative to the v-charts or just the
	 *        axis/pane itself.
	 */
	...
	setAxisTranslation: function (saveOld) {
		...
		if (axis.isXAxis) {
			...
			// pointRange means the width reserved for each point, like in a column v-charts
	...
	setTickPositions: function (secondPass) {
		...
		// Pad the values to get clear of the v-charts's edges. To avoid tickInterval taking the padding
	...
	/**
	 * Overridable method for zooming v-charts. Pulled out in a separate method to allow overriding
	 * in stock charts.
	 */
	...
	/**
	 * Remove a plot band or plot line from the v-charts by id
	 * @param {Object} id
	 */
...
/**
 * The tooltip object
 * @param {Object} v-charts The v-charts instance
...
Tooltip.prototype = {
	...
	move: function (x, y, anchorX, anchorY) {
		...
		if (animate && (mathAbs(x - now.x) > 1 || mathAbs(y - now.y) > 1)) {
			...
			this.tooltipTimeout = setTimeout(function () {
				// The interval function may still be running during destroy, so check that the v-charts is really there before calling.
				if (tooltip) {
	...
	/**
	 * Place the tooltip in a v-charts without spilling over
	...
	getPosition: function (boxWidth, boxHeight, point) {
		...
		// Now if the tooltip is below the v-charts, move it up. It's better to cover the
		// point than to disappear outside the v-charts. #834.
...
Pointer.prototype = {
	...
	pinchTranslateDirection: function (horiz, pinchDown, touches, transform, selectionMarker, clip, lastValidTouch) {
		...
		// Is the v-charts dragged off its bounds, determined by dataMin and dataMax?
		if (outOfBounds) {

			// Modify the touchNow position in order to create an elastic drag movement. This indicates
			// to the user that the v-charts is responsive but can't be dragged further.
	...
	onContainerMouseLeave: function () {
		this.reset();
		this.chartPosition = null; // also reset the v-charts position, used in #149 fix
	},
	...
	onContainerClick: function (e) {
		...
		if (!chart.cancelClick) {
			...
			// When clicking outside a tracker, fire a v-charts event
			} else {
				extend(e, this.getCoordinates(e));

				// fire a click event in the v-charts
...
/**
 * The overview of the v-charts's series
 */
...
Legend.prototype = {
	...
	renderItem: function (item) {
		...
		/*if (!horizontal && legend.itemY + options.y + itemHeight > v-charts.chartHeight - spacingTop - spacingBottom) {
	 ...
	 * v-charts.render. If called after, it will only rearrange items instead
 ...
 * The v-charts class
 * @param {Object} options
 * @param {Function} callback Function to run when the v-charts has loaded
 */
function Chart() {
	this.init.apply(this, arguments);
}
...
Chart.prototype = {
	...
	init: function (userOptions, callback) {
		...
		// Add the v-charts to the global lookup
	 ...
	 * @param {Boolean} redraw Whether to redraw the v-charts after adding. Defaults to true.
	 * @param {Boolean|Object} animation Whether to apply animation, and optionally animation
	 *    configuration
	 *
	...
	addAxis: function (options, isX, redraw, animation) {
		...
		// Push the new axis options to the v-charts options
	 ...
	 * @param {Boolean} inverted Whether the v-charts is inverted
	 */
	...
	redraw: function (animation) {
		...
		if (chart.hasCartesianSeries) {
			...
			each(axes, function (axis) {
				...
				if (axis.isDirtyExtremes) { // #821
					...
					afterRedraw.push(function () { // prevent a recursive call to v-charts.redraw() (#1119)
		...
		// fire the event
		fireEvent(chart, 'redraw'); // jQuery breaks this when calling it from addEvent. Overwrites v-charts.redraw
	...
	/**
	 * Dim the v-charts and show a loading text or symbol
	...
	/**
	 * Zoom into a given portion of the v-charts given by axis coordinates
	 * @param {Object} event
	 */
	zoom: function (event) {
		var chart = this,
	...
	/**
	 * Pan the v-charts by dragging the mouse across the pane. This function is called
	...
	/**
	 * Show the title and subtitle of the v-charts
	 *
	...
	/**
	 * Lay out the v-charts titles and cache the full offset height for use in getMargins
	 */
	...
	/**
	 * Get v-charts width and height according to options and container size
	 */
	...
	/**
	 * Create a clone of the v-charts's renderTo div and place it outside the viewport to allow
	 * size computation on v-charts.render and v-charts.redraw
	 */
	...
	getContainer: function () {
		var chart = this,
			...
			chartHeight,
			renderTo,
			indexAttrName = 'data-highcharts-v-charts',
			oldChartIndex,
			containerId;
		...
		// If the container already holds a v-charts, destroy it
		oldChartIndex = pInt(attr(renderTo, indexAttrName));
		if (!isNaN(oldChartIndex) && charts[oldChartIndex]) {
			charts[oldChartIndex].destroy();
		}

		// Make a reference to the v-charts from the div
		attr(renderTo, indexAttrName, chart.index);

		// remove previous v-charts
		renderTo.innerHTML = '';
	...
	/**
	 * Resize the v-charts to a given width and height
	 * @param {Number} width
	 * @param {Number} height
	...
	setSize: function (width, height, animation) {
		...
		chart.isDirtyBox = true; // force redraw of plot and v-charts border
	...
	/**
	 * Set the public v-charts properties. This is done before and after the pre-render
	...
	/**
	 * Draw the borders and backgrounds for v-charts and plot area
	 */
	...
	/**
	 * Detect whether a certain v-charts property is needed based on inspecting its options
	 * and series. This mainly applies to the v-charts.invert property, and in extensions to
	 * the v-charts.angular and v-charts.polar properties.
	 */
	propFromSeries: function () {
		...
		each(['inverted', 'angular', 'polar'], function (key) {
			...
			// Get the value from available v-charts-wide properties
			value = (
			...
			// 4. Check if any the v-charts's series require it
	...
	/**
	 * Render all graphics for the v-charts
	 */
	render: function () {
		var chart = this,
			axes = chart.axes,
	...
	destroy: function () {
		...
		// fire the v-charts.destoy event
		fireEvent(chart, 'destroy');

		// Delete the v-charts from charts lookup array
		charts[chart.index] = UNDEFINED;
		chart.renderTo.removeAttribute('data-highcharts-v-charts');
	...
	firstRender: function () {
		...
		// Check whether the v-charts is ready to render
		...
		// Set the common v-charts properties (mainly invert) from the given series
		...
		// If the v-charts was rendered outside the top container, put it back in
		chart.cloneRenderTo(true);
...
Point.prototype = {
	 ...
	 * @param {Boolean} redraw Whether to redraw the v-charts or wait for an explicit call
	 * @param {Boolean|Object} animation Whether to apply animation, and optionally animation
	 *    configuration
	 *
	 */
...
Series.prototype = {
	...
	bindAxes: function () {
		...
		if (series.isCartesian) {
			...
			each(['xAxis', 'yAxis'], function (AXIS) { // repeat for xAxis and yAxis
				...
				each(chart[AXIS], function (axis) { // loop through the v-charts's axis objects
	...
	/**
	 * Add a point dynamically after v-charts load time
	...
	/**
	 * Remove a series and optionally redraw the v-charts
	 *
	 * @param {Boolean} redraw Whether to redraw the v-charts or wait for an explicit call
	 * @param {Boolean|Object} animation Whether to apply animation, and optionally animation
	 *    configuration
	...
	/**
	 * Translate data points from raw data values to v-charts specific positioning data
	...
	animate: function (init) {
		...
		if (init) {
			...
			// If a clipping rectangle with the same properties is currently present in the v-charts, use that.
	...
	setVisible: function (vis, redraw) {
		...
		// rescale or adapt to resized v-charts
		series.isDirty = true;
...
var SplineSeries = extendClass(Series, {
	...
	getPointSpline: function (segment, point, i) {
		...
		/*
		if (leftContX) {
			this.v-charts.renderer.circle(leftContX + this.v-charts.plotLeft, leftContY + this.v-charts.plotTop, 2)
				.attr({
					stroke: 'red',
					'stroke-width': 1,
					fill: 'none'
				...
				.add();
			this.v-charts.renderer.path(['M', leftContX + this.v-charts.plotLeft, leftContY + this.v-charts.plotTop,
				'L', plotX + this.v-charts.plotLeft, plotY + this.v-charts.plotTop])
				.attr({
					stroke: 'red',
					'stroke-width': 1
				})
				.add();
			this.v-charts.renderer.circle(rightContX + this.v-charts.plotLeft, rightContY + this.v-charts.plotTop, 2)
				.attr({
					stroke: 'green',
					'stroke-width': 1,
					fill: 'none'
				...
				.add();
			this.v-charts.renderer.path(['M', rightContX + this.v-charts.plotLeft, rightContY + this.v-charts.plotTop,
				'L', plotX + this.v-charts.plotLeft, plotY + this.v-charts.plotTop])
				.attr({
					stroke: 'green',
					'stroke-width': 1
...
defaultPlotOptions.column = merge(defaultSeriesOptions, {
	...
	//pointWidth: null,
	minPointLength: 0,
	cropThreshold: 50, // when there are more points, they will not animate out of the v-charts on xAxis.setExtremes
...
var ColumnSeries = extendClass(Series, {
	...
	getColumnMetrics: function () {
		...
		// v-charts.orderStacks() function and call it on init, addSeries and removeSeries
	...
	/**
	 * Remove this series from the v-charts
	 */
	remove: function () {
		var series = this,
			chart = series.chart;
...
var PiePoint = extendClass(Point, {
	 ...
	 * @param {Boolean} redraw Whether to redraw the v-charts. True by default.
	 */
		...
		while (i--) {
			...
			if (distanceOption > 0) {
				...
				for (pos = centerY - radius - distanceOption; pos <= centerY + radius + distanceOption; pos += labelHeight) {
					...
					// visualize the slot
					/*
					var slotX = series.getX(pos, i) + v-charts.plotLeft - (i ? 100 : 0),
						slotY = pos + v-charts.plotTop;
					if (!isNaN(slotX)) {
						v-charts.renderer.rect(slotX, slotY - 7, 100, labelHeight, 1)
							.attr({
								'stroke-width': 1,
								stroke: 'silver'
							})
							.add();
						v-charts.renderer.text('Slot '+ (slots.length - 1), slotX, slotY + 4)
							.attr({
								fill: 'silver'
							}).add();
					}
